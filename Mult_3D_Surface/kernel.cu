...
int main(int argc, char* argv[])
{
	...
	copyparms_C.dstPtr = make_cudaPitchedPtr((float*)C_host,ancho*sizeof(float),ancho,alto);
	copyparms_C.kind = cudaMemcpyDeviceToHost;
	


	// 3D copy from host_CubeMatrix to cudaArray
	//copyparms_C.srcArray = make_((void*)cudaArray_C,ancho*sizeof(float),ancho,alto);
