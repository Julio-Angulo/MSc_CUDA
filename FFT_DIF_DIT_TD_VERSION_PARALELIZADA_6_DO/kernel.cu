...
///(18/01/2017)
///Grafica en Matlab los tiempos de ejecución, considerando Radix-2. N = 2^20, Li = N, Lo= {2, 4,…,N}. (precisión doble).
...
int main()
{
	...
	if(device == 1)
	{
		...
		da = fopen("Tiempos_N20_LiN_LoVARIA_CUDA_GTX970_DO.bin","a+b"); //Crea o sobre escribe archivo
	}
	if(device == 0)
	{
		printf("\n\n---DEVICE = TESLA K20---\n\n");
		da = fopen("Tiempos_N20_LiN_LoVARIA_CUDA_TESLAK20c_DO.bin","a+b"); //Crea o sobre escribe archivo
	}
	//////////////////////////////////////////////////////////////////////////
	...
	for(i_N = N_max;i_N <= N_max;i_N++)
    {
		...
		for(j_res=(int )pow(2,Li_max);j_res <= (int )pow(2,Li_max);j_res++)
        {
			...
			for(k_res=1;k_res <= N_max;k_res++)
            {
				...
				//Se imprimen los valores de los factores Dip, Dop y P
