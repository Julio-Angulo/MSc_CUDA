///VERSION CUDA paralelo 2D versión 2
...
//Kernel
__global__ void QR_kernel(int M,int N,int t)
{
	float r,temp,data_1,data_2,data_3,data_4,data_5,data_6,aux_1,aux_2,aux_3,aux_4;
	int p1,p2,k,i,j;
	int aux_inf_1,aux_sup_1,lim_inf_p2,lim_sup_p2;

	int x = blockDim.x *blockIdx.x + threadIdx.x;
	int y = blockDim.y *blockIdx.y + threadIdx.y;

	aux_inf_1 = fmaxf(0,t-N+2+(1-M));
	aux_sup_1 = fminf(M-2,floorf((t+M-1+(1-M))/3));
	...
	for(p1 = aux_inf_1;p1 <=aux_sup_1;p1++)
	{
		...
		//printf(" p1 = %d\n ",p1);
		lim_inf_p2 = fmaxf(0,(t-2*p1-N+1+(1-M))-(1-M));
		lim_sup_p2 = fminf(t-3*p1+(1-M)-(1-M),-1-p1-(1-M));

		if( (x == p1) && (y >= lim_inf_p2 && y <= lim_sup_p2 ))
		{
			p1=x;
			p2=y;
			k=p1;
...
int main()
{
	int i,j,loop;
	int N,M,k,r,t;
	int inf_p1,sup_p1,inf_p2,sup_p2;
	float suma;
	...
	for(i=1;i<=muestras;i++)
    {
        ...
        for(j=0;j<loop;j++)
        {
			...
			else
			{
				blockDim.x = 32;
				blockDim.y = 32;
				gridDim.x = ceil(M-1/blockDim.x);
				gridDim.y = ceil(M-1/blockDim.y);
			}
			...
			for(t = 0; t <= fminf(M+N-4-(1-M),(floorf((M+3*N-7)/2))-(1-M));t++)
			{

				QR_kernel<<<gridDim,blockDim >>>(M,N,t);
