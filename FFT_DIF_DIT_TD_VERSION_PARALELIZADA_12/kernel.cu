...
///(31/08/2016)
///Ésta versión sirve para graficar en matlab los tiempos de ejecución, considerando (RADIX-3) N = 3^13, Li = N y Lo = Varía
...
int main()
{
	...
	if(device == 0)
	{
		...
		da = fopen("Tiempos_N13_LiN_LoVARIA_CUDA_GTX970.bin","a+b"); //Crea o sobre escribe archivo
	}
	if(device == 1)
	{
		printf("\n\n---DEVICE = TESLA K20---\n\n");
		da = fopen("Tiempos_N13_LiN_LoVARIA_CUDA_TESLAK20c.bin","a+b"); //Crea o sobre escribe archivo
	}
