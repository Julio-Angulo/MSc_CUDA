...
///(26/08/2016)
///Ésta versión sirve para graficar en matlab los errores absolutos y relativos (RADIX-2) 2^1 - 2^10. Los arreglos x_host y W_host se generan en el device y la función asign_rap se ejecuta en el device
...
//////////////////////////////////////////////////////////////////////////
void vector_entrada_xn(int N_max);
__global__ void init(unsigned int seed, curandState_t *states,int N_max); 
__global__ void entrada_xn_kernel(int N_max,float *alea_real,float *alea_imag,cuFloatComplex *x_device,FILE *dc,curandState_t *states);
void arreglo_W(int N);
__global__ void arreglo_W_kernel(int N,cuFloatComplex *W_device);
...
int main()
{
	int i,j,i_N,l_res,j_res,k_res,incremento_j;
	//float suma;
	...
	//Se generan en el device los valores del vector de entrada x[n] 
	vector_entrada_xn(N_max);
	fclose(dc);
	...
	for(i_N = 1;i_N <= 10;i_N++)
    {
		...
		//Se genera el arreglo W[N] en el device
        arreglo_W(N);
...
void vector_entrada_xn(int N_max)
{
	...
	//we will stor3e a random state for every thread
	curandState_t *states;

	//Allocate space on the GPU for the random states
	cudaMalloc((void**) &states,N_max*sizeof(curandState_t));
	
	//Se reserva memoria para x_device
	cudaMalloc((void**)&x_device,N_max*sizeof(cuFloatComplex));
	cudaMalloc((void**)&alea_real,N_max*sizeof(float));
	cudaMalloc((void**)&alea_imag,N_max*sizeof(float));	
	...
	{
		blockDim.x = 1024;
		gridDim.x = (unsigned int) (ceilf((float)N_max/(float)blockDim.x));
	}

	//Invoke the GPU to initialize all of the random states
	init<<<gridDim,blockDim>>>(time(0),states,N_max);
	
	//Esperar que el kernel termine de ejecutarse totalmente
	cudaDeviceSynchronize();

	//Lanzamiento del kernel "entrada_xn_kernel"
	entrada_xn_kernel<<<gridDim,blockDim>>>(N_max,alea_real,alea_imag,x_device,dc,states);
...
__global__ void entrada_xn_kernel(int N_max,float *alea_real,float *alea_imag,cuFloatComplex *x_device,FILE *dc,curandState_t *states)
{
	...
	if(i < N_max)
    {
        alea_real[i]=curand(&states[i]) % 11;
        //alea_real[i]=i+1;
        alea_imag[i]=curand(&states[i]) % 11;
        //alea_imag[i]=0;
        fprintf(dc,"%d %d\n",alea_real[i],alea_imag[i]);
		x_device[i] = make_cuFloatComplex((float)alea_real[i],(float)alea_imag[i]);
    }
...
void arreglo_W(int N)
{
	//Se reserva memoria para W_host
	...
	{
		blockDim.x = 1024;
		gridDim.x = (unsigned int) (ceilf((float)N/(float)blockDim.x));
	}

	//Lanzamiento del kernel "entrada_xn_kernel"
	arreglo_W_kernel<<<gridDim,blockDim>>>(N,W_device);

	//Esperar que el kernel termine de ejecutarse totalmente
	cudaDeviceSynchronize();
}

//Función kernel para W_device
__global__ void arreglo_W_kernel(int N,cuFloatComplex *W_device)
{
	//Threads
	...
	if(n < N)
	{
		W_device[n] = make_cuFloatComplex((float)cos((2*CUDART_PI*(n+1))/N),(float)(-1)*sin((2*CUDART_PI*(n+1))/N));
	}
}
