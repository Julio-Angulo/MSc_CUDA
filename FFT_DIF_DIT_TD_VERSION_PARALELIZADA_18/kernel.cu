...
///(09/09/2016)
///Ésta versión sirve para detectar error en las etapas con ayuda de las flags, considerando (RADIX-3) N = 3^13, Li = Varía y Lo = 43
...
int main()
{
	...
	FILE *da;
	da = fopen("Tiempos_N13_LiVARIA_Lo43_CUDA.bin","a+b"); //Crea o sobre escribe archivo
	...
	for(i_N = N_max;i_N <= N_max;i_N++)
    {
		...
		for(j_res=1;j_res <= N_max;j_res++)
        {
			...
			for(k_res=Lo_max;k_res <= Lo_max;k_res++)
            {
                ...
                db_open = fopen("Entrada_real_N13_LiVARIA_Lo43_C.bin","rb");
                dc_open = fopen("Entrada_imag_N13_LiVARIA_Lo43_C.bin","rb");
				...
				promedio[j_res-1] = suma/(float)loop;
				fclose(db_open);
                fclose(dc_open);
			}
