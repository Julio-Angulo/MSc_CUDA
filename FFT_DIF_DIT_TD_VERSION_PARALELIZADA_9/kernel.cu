...
///(31/08/2016)
///Ésta versión sirve para graficar en matlab los tiempos de ejecución, considerando (RADIX-2) N = 2^20, Li = Varía y Lo = N
...
int main()
{
	...
	if(device == 0)
	{
		...
		da = fopen("Tiempos_N20_LiVARIA_LoN_CUDA_GTX970.bin","a+b"); //Crea o sobre escribe archivo
	}
	if(device == 1)
	{
		printf("\n\n---DEVICE = TESLA K20---\n\n");
		da = fopen("Tiempos_N20_LiVARIA_LoN_CUDA_TESLAK20c.bin","a+b"); //Crea o sobre escribe archivo
	}
	...
	for(i_N = N_max;i_N <= N_max;i_N++)
    {
        ...
        {
            Li=(int )pow(2,j_res);
			for(k_res=N_max;k_res <= N_max;k_res++)
            {
				Lo=(int )pow(2,k_res);
				printf("\n Li = %d  Lo = %d",Li,Lo);
